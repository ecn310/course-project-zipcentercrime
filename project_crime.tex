...
Figure~\ref{fig:Figure3} represents the mean calls per SATC per kilometer squared by distance group in a bar graph.These are the average calls for the total 911 call set per SATC. Figure~\ref{fig:Figure3} uses the same data as Table~\ref{tabl:Table1} and represent the same means and significance levels used. Confidence intervals of 90\% for the mean calls are overlaid on top of their respective distance group. 

The confidence intervals of 250-500 meters and 750-1000 meters significantly overlap, as the confidence interval of the larger distance group overlaps the mean, so consequently the confidence interval of the smaller distance group. This shows that there is no significance in the differences of means presented in the data. This could be due to The evidence of any difference could be by random chance. The confidence intervals visually include the mean of the distance group to the left, showing that the mean values  The other confidence intervals do overlap with the sequential intervals, but not visually as significantly as in the two comparisons mentioned above. These observations additionally support the p-values observed in Table 1. 
