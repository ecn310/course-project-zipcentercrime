...
twoway (scatter latitude longitude, sort msize(medsmall)) (scatter latitude longitude if year_num == 2017, sort mcolor(green%50) msize(medsmall)), legend(order(1 "All Years" 2 "2017 Only")) name(Scatter)
...
twoway (scatter latitude longitude, msymbol(triangle) sort mcolor(yellow%75) msize(medsmall)) (scatter latitude longitude if year == 2017, msymbol(triangle) sort mcolor(red%75) msize(medsmall)), yscale(range(42 42.8)) yscale(extend fextend) ylabel(42(0.1)42.8) ymtick(, valuelabel alternate nogmin gmax gextend) xscale(range(-84.5 -82.5)) xlabel(-84.5(.75)-82.5) legend(order(1 "All Years" 2 "2017 Only")) name(Scatter2)  
